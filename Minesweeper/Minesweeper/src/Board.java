...
public class Board extends JPanel {
    ...
    // Return if the current board has been won or not
    // Output: Boolean
    ...
    // Input: Cell
    // Check if the given cell is on the board or not
    // Output: Boolean
        ...
        public void mousePressed(MouseEvent e) {
            ...
            if((x < COLUMNS * CELL_SIZE) && (y < ROWS * CELL_SIZE)) {
                ...
                if (e.getButton() == MouseEvent.BUTTON3) {  //If right clicked
                    if(!clicked.COVERED) {                  //Don't do anything if its already uncovered
                        return;
                    }
                    if (clicked.COVERED && !clicked.FLAGGED) {      //If covered and not flagged
                        if (minesLeft > 0) {
                            clicked.FLAGGED = true;                 //flag it
                            minesLeft--;
                        ...
                        clicked.FLAGGED = false;                    //Unflag it if trying to flag a flag
                        minesLeft++;
                        String msg = Integer.toString(minesLeft);
                        STATUS.setText(msg);
                        REPAINT = true;
                ...
                if(e.getButton() == MouseEvent.BUTTON1) {       //If clicked
                    ...
                    if(clicked.COVERED) {
                        ...
                        if (clicked.N > 0) {
                            ...
                            extendZero(clicked);              //Open up the board
                            REPAINT = true;
                        }
                    }
                }
            }
            //If smile clicked reset the board
            if((x > (WIDTH/2) - 13) && (x <= (WIDTH/2)) && (y >= HEIGHT+25) && (y < HEIGHT+50)) {
